...
export function ProjectsPreviewModal() {
  return (
    <ModalShell
      title={
        <div className="flex items-center gap-2">
          <span>Projects</span>
          <button
            onClick={() => window.open('https://portfolio-chanukyachintada.vercel.app/projects.html', '_blank')}
            className="text-gray-500 hover:text-black transition flex items-center justify-center p-1"
            title="View full projects page"
          >
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-wrap gap-1">
                {project.tags.slice(0, 3).map((tag) => (
                  <span key={tag} className="text-xs px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded-md">
                    {tag}
                  </span>
                ))}
              </div>
